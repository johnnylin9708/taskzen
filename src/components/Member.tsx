...
const Member = () => {
  ...
  const handleGetTeamsAPI = async () => {
    ...
    const teams: { id: number; name: string; description: string }[] =
      response.data;
    setTeamRows(teams);
  };
  ...
  return (
    <>
      <Box sx={{ display: "flex", width: "100%" }}>
        <Paper sx={{ width: "30%", margin: 2, overflow: "hidden" }}>
                  ...
                  teamRows.map((team) => {
                    return (
                      <TableRow
                        ...
                        onClick={() => handleTeamMember(team.id)}
                      >
                        {columns.map((column) => {
      ...
      <TeamCreationDialog
        ...
        handleGetTeamsAPI={handleGetTeamsAPI}
      />
    </>
  );
};
